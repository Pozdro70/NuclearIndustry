...
public class StillScreen extends AbstractContainerScreen<StillMenu> {
    ...
    private static final ResourceLocation TEXTURE = new ResourceLocation(NuclearIndustry.MODID,
            "textures/gui/still_gui.png");

    private EnergyInfoArea energyInfoArea;

    private FluidTankRenderer tankRenderer1;
    private FluidTankRenderer tankRenderer2;
    public StillScreen(StillMenu pMenu, Inventory pPlayerInventory, Component pTitle) {
    ...
    private void renderFluidAreaTooltips(PoseStack pPoseStack, int pMouseX, int pMouseY, int x, int y) {
        if(isMouseAboveAreaT1(pMouseX, pMouseY, x, y, 29, 8)) {
    ...
    protected void renderBg(PoseStack pPoseStack, float pPartialTick, int pMouseX, int pMouseY) {
        ...
        tankRenderer1.render(pPoseStack, x+29,y+8,menu.getFluidInINTank());
        tankRenderer2.render(pPoseStack, x+116,y+8,menu.getFluidInOUTTank());
    }

    private void renderProgressArrow(PoseStack pPoseStack, int x, int y) {
        if(menu.isCrafting()) {
            blit(pPoseStack, x + 46, y + 12, 176, 0,menu.getScaledProgress(),39 );
        }
    }
