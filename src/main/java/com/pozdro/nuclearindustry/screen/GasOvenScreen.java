...
public class GasOvenScreen extends AbstractContainerScreen<GasOvenMenu> {
    ...
    private FluidTankRenderer tankRenderer3;

    public GasOvenScreen(GasOvenMenu gasOvenMenu, Inventory inventory, Component component) {
        super(gasOvenMenu, inventory, component);
    }
    ...
    private void assignFluidRenderer() {
        ...
        tankRenderer3 = new FluidTankRenderer(64000,true,14,63);
    }
    ...
    private void assignEnergyInfoArea() {
        ...
        energyInfoArea = new EnergyInfoArea(x + 164, y + 2, menu.blockEntity.getEnergyStorage());
    }
    ...
    private void renderFluidAreaTooltips(PoseStack pPoseStack, int pMouseX, int pMouseY, int x, int y) {
        ...
        if(isMouseAboveAreaT3(pMouseX, pMouseY, x, y, 10, 6)) {
            renderTooltip(pPoseStack, tankRenderer3.getTooltip(menu.getFluidInIN1Tank(), TooltipFlag.Default.NORMAL),
                    Optional.empty(), pMouseX - x, pMouseY - y);
        }
    }
    ...
    protected void renderBg(PoseStack stack, float pPartialTick, int pMouseX, int pMouseY) {
        ...
        tankRenderer3.render(stack, x+10,y+6,menu.getFluidInIN1Tank());
    }

    private void renderProgressArrow(PoseStack pPoseStack, int x, int y) {
        if(menu.isCrafting()) {
            blit(pPoseStack, x + 43, y + 8, 0, 176, menu.getScaledProgress(), 43);
        }
    }
    ...
    private boolean isMouseAboveAreaT3(int pMouseX, int pMouseY, int x, int y, int offsetX, int offsetY) {
        return MouseUtil.isMouseOver(pMouseX, pMouseY, x + offsetX, y + offsetY, tankRenderer3.getWidth(), tankRenderer3.getHeight());
    }
